...
static int ndisc_constructor(struct neighbour *neigh)
{
	...
	if (!dev->header_ops) { //不需要ARP的设备
		neigh->nud_state = NUD_NOARP;
		neigh->ops = &ndisc_direct_ops;
		neigh->output = neigh_direct_output;
	} else {
		if (is_multicast) { //组播目的地址不需要AR,直接将IP映射为MAC
			neigh->nud_state = NUD_NOARP;
			ndisc_mc_map(addr, neigh->ha, dev, 1);
		} else if (dev->flags&(IFF_NOARP|IFF_LOOPBACK)) { //接口本身不支持ARP或者LOOPBACK接口
			neigh->nud_state = NUD_NOARP;
			memcpy(neigh->ha, dev->dev_addr, dev->addr_len); //将目的MAC直接设置为本接口MAC地址
			if (dev->flags&IFF_LOOPBACK)
				neigh->type = RTN_LOCAL;
		} else if (dev->flags&IFF_POINTOPOINT) { //对于P2P接口,也不需要ARP.
			neigh->nud_state = NUD_NOARP;
			memcpy(neigh->ha, dev->broadcast, dev->addr_len);
		}
		if (dev->header_ops->cache) //接口设备支持Cache
