...
#define _DEVINC( net, statname, mod, idev, field )                             \
    ( {                                                                        \
        ...
        mod##SNMP_INC_STATS64( ( net )->mib.statname##_statistics,             \
                               ( field ) );                                    \
    } )
...
#define _DEVINC_ATOMIC_ATOMIC( net, statname, idev, field )                    \
    ( {                                                                        \
        ...
        if ( likely( _idev != NULL ) )                                         \
            SNMP_INC_STATS_ATOMIC_LONG( ( _idev )->stats.statname##dev,        \
                                        ( field ) );                           \
        SNMP_INC_STATS_ATOMIC_LONG( ( net )->mib.statname##_statistics,        \
                                    ( field ) );                               \
    } )
...
#define _DEVADD( net, statname, mod, idev, field, val )                        \
    ( {                                                                        \
        struct inet6_dev *_idev = ( idev );                                    \
        if ( likely( _idev != NULL ) )                                         \
            mod##SNMP_ADD_STATS( ( _idev )->stats.statname, ( field ),         \
                                 ( val ) );                                    \
        mod##SNMP_ADD_STATS( ( net )->mib.statname##_statistics, ( field ),    \
                             ( val ) );                                        \
    } )
...
#define _DEVUPD( net, statname, mod, idev, field, val )                        \
    ( {                                                                        \
        struct inet6_dev *_idev = ( idev );                                    \
        if ( likely( _idev != NULL ) )                                         \
            mod##SNMP_UPD_PO_STATS( ( _idev )->stats.statname, field,          \
                                    ( val ) );                                 \
        mod##SNMP_UPD_PO_STATS( ( net )->mib.statname##_statistics, field,     \
                                ( val ) );                                     \
    } )

/* MIBs */

#define IP6_INC_STATS( net, idev, field ) _DEVINC( net, ipv6, , idev, field )
...
static inline void txopt_put( struct ipv6_txoptions *opt ) {
    if ( opt && atomic_dec_and_test( &opt->refcnt ) ) kfree_rcu( opt, rcu );
}
...
struct ipv6_txoptions *ipv6_renew_options( struct sock *          sk,
                                           ...
                                           int newoptlen );
struct ipv6_txoptions *ipv6_renew_options_kern( struct sock *          sk,
                                                struct ipv6_txoptions *opt,
                                                int                    newtype,
...
static inline int __ipv6_addr_src_scope( int type ) {
    return ( type == IPV6_ADDR_ANY ) ? __IPV6_ADDR_SCOPE_INVALID
                                     : ( type >> 16 );
}
...
#else
    return !!(
        ( ( a1->s6_addr32[ 0 ] ^ a2->s6_addr32[ 0 ] ) & m->s6_addr32[ 0 ] ) |
...
static inline void __ipv6_addr_set_half( __be32 *addr, __be32 wh, __be32 wl ) {
    ...
    if ( __builtin_constant_p( wh ) && __builtin_constant_p( wl ) ) {
        *(__force u64 *) addr =
            ( ( __force u64 )( wh ) << 32 | ( __force u64 )( wl ) );
        return;
    }
#elif defined( __LITTLE_ENDIAN )
    if ( __builtin_constant_p( wl ) && __builtin_constant_p( wh ) ) {
        *(__force u64 *) addr =
            ( ( __force u64 )( wl ) << 32 | ( __force u64 )( wh ) );
        return;
    }
#endif
#endif
...
static inline bool ipv6_addr_any( const struct in6_addr *a ) {
    ...
    return ( ul[ 0 ] | ul[ 1 ] ) == 0UL; // Two 64 bit long ---both 0
#else
    return ( a->s6_addr32[ 0 ] | a->s6_addr32[ 1 ] | a->s6_addr32[ 2 ] |
             a->s6_addr32[ 3 ] ) == 0; // Four 32 bit long --- both 0
#endif
}
...
static inline u32 __ipv6_addr_jhash( const struct in6_addr *a,
    ...
    return jhash_3words( v, (__force u32) a->s6_addr32[ 2 ],
                         (__force u32) a->s6_addr32[ 3 ], initval );
}
...
static inline bool ipv6_addr_v4mapped( const struct in6_addr *a ) {
    return (
               ...
               *(unsigned long *) a |
#else
               (__force unsigned long) ( a->s6_addr32[ 0 ] |
                                         a->s6_addr32[ 1 ] ) |
#endif
...
static inline bool ipv6_addr_is_multicast( const struct in6_addr *addr ) {
    return ( addr->s6_addr32[ 0 ] & htonl( 0xFF000000 ) ) ==
           htonl( 0xFF000000 );
}
...
static inline int __ipv6_addr_diff64( const void *token1, const void *token2,
    ...
    for ( i = 0; i < addrlen; i++ ) {
        __be64 xb = a1[ i ] ^ a2[ i ];
        if ( xb ) return i * 64 + 63 - __fls( be64_to_cpu( xb ) );
    }
...
__be32 ipv6_select_ident( struct net *net, const struct in6_addr *daddr,
...
static inline __be32 ip6_make_flowinfo( unsigned int tclass,
                                        __be32       flowlabel ) {
...
int ip6_dst_lookup( struct net *net, struct sock *sk, struct dst_entry **dst,
                    struct flowi6 *fl6 );
struct dst_entry *ip6_dst_lookup_flow( const struct sock *    sk,
                                       struct flowi6 *        fl6,
...
int __ip6_datagram_connect( struct sock *sk, struct sockaddr *addr,
                            int addr_len );
int ip6_datagram_connect( struct sock *sk, struct sockaddr *addr,
                          int addr_len );
